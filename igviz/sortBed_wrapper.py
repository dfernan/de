...
def run_sortBed(doe_csv, in_dir, header_name_of_exp_id, extension, logs_dir, remove, execute, job_ext):
    qname = 'priority'
    mem_usage = '20000'
    ...
    for exp_name, bed_in_fn in bed_fn_dict.iteritems():
        bsubcmd = myos.create_bsub_string_no_rm_logs_dir(logs_dir, exp_name+job_ext, qname = qname, mem_usage = mem_usage)
...
def main():
    ...
    parser.add_option('-j', '--job_ext', action = "store", default='')
    parser.add_option('-r', '--rm', action = "store_true", default=False)
    parser.add_option('-x', '--execute', action = "store_true", default=False)
    (options, args) = parser.parse_args()
    run_sortBed(options.doe_csv_fn, options.in_dir_bam_files, options.header_name_of_exp_id, options.extension_after_exp_id, options.logs_dir, options.rm, options.execute, options.job_ext)
    return 0
