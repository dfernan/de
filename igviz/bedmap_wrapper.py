...
def main():
    ...
    parser.add_option('-r', '--in_reference_fn', action = "store", help="in case of mapping reads to vcf, reference_fn = vcf_fn")
    parser.add_option('-d', '--doe_csv_fn', action = "store")
    parser.add_option('--header_name_of_exp_id', action = "store")
    parser.add_option('-i', '--in_map_dir', action = "store", help = "in case of mapping reads to vcf, map_dir is the directory that contains all bam2bed (sorted) files")
    ...
    run_bedmap(options.in_reference_fn, options.doe_csv_fn, options.header_name_of_exp_id, options.in_map_dir, options.extension_after_exp_id, options.bedmap_options, options.out_dir, options.logs_dir, options.execute)
    return 0
